...
use crate::layers::bio::{BlockRing, BlockSet, Buf};
use crate::layers::crypto::{CryptoBlob, CryptoChain, Key, Mac};
use crate::prelude::*;
    ...
    D: BlockSet,
{
    ...
    pub fn keys(&self) -> [Key; 2] {
        [self.blobs[0].key().clone(), self.blobs[1].key().clone()]
    }
}
...
impl<'a, E: Edit<S>, S> Iterator for RecordSlice<'a, E, S> {
