...
///
/// Use `Opaque` type to wrap the `struct task_struct`, which is meant to be
/// used with FFI objects that are never interpreted by Rust code.
#[repr(transparent)]
...
impl<T> ThreadItem<T> {
    ...
    pub fn execute(&mut self) {
        let closure = self.func.take().expect("take closure more than once");
...
impl Thread {
        ...
        T: Send + 'static,
    {
        ...
        let item_ptr = Box::into_raw(ThreadItem::try_new(func)?);
